...
function Prospecting.AddTarget(areaName, x, y, z, data)
    exports['ts-prospecting']:AddProspectingTarget(areaName, x, y, z, data)
end

function Prospecting.GetProspectingTargets()
    return exports['ts-prospecting']:GetProspectingTargets()
end

function Prospecting.AddTargets(list)
    exports['ts-prospecting']:AddProspectingTargets(list)
end

function Prospecting.StartProspecting(player)
    exports['ts-prospecting']:StartProspecting(player)
end

function Prospecting.StopProspecting(player)
    exports['ts-prospecting']:StopProspecting(player)
end

function Prospecting.IsProspecting(player)
    return exports['ts-prospecting']:IsProspecting(player)
end

function Prospecting.SetDifficulty(modifier)
    return exports['ts-prospecting']:SetDifficulty(modifier)
end

function Prospecting.OnStart(handler)
    AddEventHandler("ts-exports['ts-prospecting']:server:onStart", function(player)
        handler(player)
    end)
end

function Prospecting.OnStop(handler)
    AddEventHandler("ts-exports['ts-prospecting']:server:onStop", function(player, time)
        handler(player, time)
    end)
end

function Prospecting.SetHandler(handler)
    AddEventHandler("ts-exports['ts-prospecting']:server:onCollected", function(player, resource, data, x, y, z)
...
function HandleProspectingPickup(player, index, x, y, z)
    ...
    if target then
        ...
        if math.floor(dx) == math.floor(x) and math.floor(dy) == math.floor(y) and math.floor(dz) == math.floor(z) then
            ...
            TriggerEvent("ts-exports['ts-prospecting']:server:onCollected", player, resource, data, x, y, z)
        else
            local newMatch = FindMatchingPickup(x, y, z)
            if newMatch then
                HandleProspectingPickup(player, newMatch, x, y, z)
            else
                Debugging(string.format("exports['ts-prospecting']: Player %s tried to collect non-existent target at %s, %s, %s", player, x, y, z))
            end
        end
    else
        Debugging(string.format("exports['ts-prospecting']: Player %s tried to collect invalid target index %s", player, index))
    end
end

function AddProspectingTarget(areaName, x, y, z, data)
    local resource = GetInvokingResource()
    InsertProspectingTarget(resource, areaName, x, y, z, data)
end
...
function TriggerUpdate()
    ...
    TriggerClientEvent("ts-exports['ts-prospecting']:client:setTargetPool", -1, targets)
    updateTimer = nil
end
...
function UpdateProspectingTargets(player_src)
    ...
    TriggerClientEvent("ts-exports['ts-prospecting']:client:setTargetPool", player_src, targets)
end
...
function StartProspecting(player)
    if not PROSPECTING_STATUS[player] then
        TriggerClientEvent("ts-exports['ts-prospecting']:client:forceStart", player)
    end
end

function StopProspecting(player)
    if PROSPECTING_STATUS[player] then
        TriggerClientEvent("ts-exports['ts-prospecting']:client:forceStop", player)
    end
end
...
AddEventHandler("ts-exports['ts-prospecting']:server:AddProspectingTarget", function(areaName, x, y, z, data)
    AddProspectingTarget(areaName, x, y, z, data)
end)

AddEventHandler("ts-exports['ts-prospecting']:server:AddProspectingTargets", function(list)
    AddProspectingTargets(list)
end)

AddEventHandler("ts-exports['ts-prospecting']:server:StartProspecting", function(player)
    StartProspecting(player)
end)

AddEventHandler("ts-exports['ts-prospecting']:server:StopProspecting", function(player)
    StopProspecting(player)
end)

RegisterServerEvent("ts-exports['ts-prospecting']:server:userStoppedProspecting")
AddEventHandler("ts-exports['ts-prospecting']:server:userStoppedProspecting", function()
    local player = source
    if PROSPECTING_STATUS[player] then
        local time = GetGameTimer() - PROSPECTING_STATUS[player]
        PROSPECTING_STATUS[player] = nil
        TriggerEvent("ts-exports['ts-prospecting']:server:onStop", player, time)
    end
end)

RegisterServerEvent("ts-exports['ts-prospecting']:server:userStartedProspecting")
AddEventHandler("ts-exports['ts-prospecting']:server:userStartedProspecting", function()
    local player = source
    if not PROSPECTING_STATUS[player] then
        PROSPECTING_STATUS[player] = GetGameTimer()
        TriggerEvent("ts-exports['ts-prospecting']:server:onStart", player)
    end
end)

RegisterServerEvent("ts-exports['ts-prospecting']:server:userCollectedNode")
AddEventHandler("ts-exports['ts-prospecting']:server:userCollectedNode", function(index, x, y, z)
    local player = source
    if PROSPECTING_STATUS[player] then
...
RegisterServerEvent("ts-exports['ts-prospecting']:server:userRequestsLocations")
AddEventHandler("ts-exports['ts-prospecting']:server:userRequestsLocations", function()
    local player = source
